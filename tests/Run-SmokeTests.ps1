...
$oldId = "old-0001"
$project = Join-Path $OutputRoot "ProjectA"
...
Invoke-CHRSqlite -Database $db -Sql "insert into threads(id,rollout_path,created_at,updated_at,source,model_provider,cwd,title,sandbox_policy,approval_mode,tokens_used,has_user_event,archived,cli_version,first_user_message,memory_mode,created_at_ms,updated_at_ms,thread_source,preview) values('template-0001',$(Quote-CHRSql $templatePath),1,1,'vscode','codex_local_access',$(Quote-CHRSql $dbCwd),'template','{}','on-request',0,1,0,'0.test','template','enabled',1000,1000,'user','template');" | Out-Null
...
Invoke-CHRSqlite -Database $db -Sql "insert into threads(id,rollout_path,created_at,updated_at,source,model_provider,cwd,title,sandbox_policy,approval_mode,tokens_used,has_user_event,archived,cli_version,first_user_message,memory_mode,created_at_ms,updated_at_ms,thread_source,preview) values('old-0001',$(Quote-CHRSql $oldPath),1,1,'vscode','openai',$(Quote-CHRSql $dbCwd),'old title','{}','on-request',0,1,0,'0.old','old title','enabled',1000,1000,'user','old title');" | Out-Null
...
Assert-True ($provider -eq "codex_local_access") "Provider should be repaired"
...
Assert-True ($providerAfterRollback -eq "openai") "Rollback should restore provider"
